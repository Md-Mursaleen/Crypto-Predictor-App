...
export const getCryptoCoinData = async (pagenumber = 1) => {
    try {
        const response = await axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page=${pagenumber}&sparkline=true&price_change_percentage=24h&locale=en`);
        return response.data;
    } catch (e) {
        console.log(e);
    }
...
export const getWatchlistedCrypto = async (pagenumber = 1, cryptoids) => {
    try {
        const response = await axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${cryptoids}&order=market_cap_desc&per_page=50&page=${pagenumber}&sparkline=true&price_change_percentage=24h&locale=en`);
        return response.data;
    } catch (e) {
        console.log(e);
    }
